...
main :: IO ()
main = do
    contents <- BS.readFile "log_file.log"
    let logMessages = parse (BS.unpack contents)
    putStrLn "Exercise 1: Parsed log messages:"
    mapM_ print logMessages

    let messageTree = build logMessages
    putStrLn "\nExercise 3: Built message tree:"
    print messageTree

    let sortedMessages = inOrder messageTree
    putStrLn "\nExercise 4: Sorted log messages:"
    mapM_ print sortedMessages

    let severeErrors = whatWentWrong logMessages
    putStrLn "\nExercise 5: Severe errors:"
    mapM_ putStrLn severeErrors